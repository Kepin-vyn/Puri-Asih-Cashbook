...
const StatusBadge = ({ status }) => {
  const map = {
    ...
    approved:      { label: "Disetujui",            cls: "bg-blue-100 text-black ring-[#e5e5e5]" },
    rejected:      { label: "Ditolak",              cls: "bg-red-100 text-red-700 ring-red-200" },
  };
  const cfg = map[status] ?? { label: status, cls: "bg-[#fafafa] text-[#737373] ring-gray-200" };
  return (
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <h1 className="text-2xl font-bold text-black">Approval Pengeluaran</h1>
          {pendingCount > 0 && (
      ...
      <div className="flex items-center gap-3">
        ...
        {filterStatus && filterStatus !== "pending" && (
          <button onClick={() => { setFilterStatus("pending"); setPage(1); }} className="text-xs text-[#737373] hover:text-[#525252] underline">
            Kembali ke Pending
          </button>
        )}
      </div>
            ...
            <tbody className="divide-y divide-[#e5e5e5]">
              {isLoading ? (
                ...
                expenses.map((exp, idx) => {
                  ...
                  return (
                    <tr key={exp.id} className={`transition-colors ${isPending ? "bg-amber-50/40 hover:bg-amber-50" : "hover:bg-[#fafafa]"}`}>
                      ...
                      <td className="px-4 py-3 font-medium text-black whitespace-nowrap">{exp.user?.name ?? "-"}</td>
                      <td className="px-4 py-3 text-[#525252] whitespace-nowrap capitalize">{exp.shift?.type ?? "-"}</td>
                      <td className="px-4 py-3 text-black max-w-[180px] truncate" title={exp.description}>{exp.description}</td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          ...
                          <button
                            ...
                            className="flex items-center gap-1 px-2 py-1 text-xs font-medium text-black bg-[#fafafa] hover:bg-[#fafafa] rounded-lg transition-colors"
                            title="Lihat Detail"
                          >
                            <Eye size={12} />
                            Detail
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-xl  w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-5 border-b border-[#e5e5e5] sticky top-0 bg-white z-10">
              <h3 className="font-bold text-black">Detail Pengeluaran</h3>
              <button onClick={() => setDetailItem(null)} className="p-1 text-[#a3a3a3] hover:text-[#525252] rounded-lg"><X size={18} /></button>
            </div>
            <div className="p-5 space-y-3 text-sm">
              ...
              {detailItem.receipt_photo_url ? (
                <div className="mt-2">
                  <p className="text-xs font-semibold text-[#737373] uppercase tracking-wide mb-2">Bukti Struk</p>
                  {detailItem.receipt_photo_url.match(/\.(jpg|jpeg|png|gif|webp)$/i) ? (
                    <a href={detailItem.receipt_photo_url} target="_blank" rel="noopener noreferrer">
                      <img
                        ...
                        alt="Bukti struk"
                        className="w-full rounded-xl border border-[#e5e5e5] object-contain max-h-64 cursor-zoom-in hover:opacity-90 transition-opacity"
                      />
                    </a>
                  ) : (
                    <a
                      ...
                      className="flex items-center gap-2 px-3 py-2 bg-[#fafafa] border border-[#e5e5e5] rounded-xl text-sm text-black hover:bg-[#fafafa] transition-colors"
                    >
                      <FileText size={16} />
                      Lihat Dokumen Struk
                    </a>
                ...
                <div className="flex items-center gap-2 px-3 py-2 bg-[#fafafa] rounded-xl text-xs text-[#a3a3a3]">
                  <ImageIcon size={14} />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-xl  w-full max-w-md p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold text-black">Tolak Pengeluaran</h3>
              <button
            ...
            <div className="mb-4">
              ...
              <textarea
                ...
                className={`w-full px-3 py-2 border rounded-xl text-sm focus:outline-none  focus:ring-red-400 resize-none ${
                  rejectReasonErr ? "border-red-400 bg-red-50" : "border-[#e5e5e5]"
                }`}
              />
